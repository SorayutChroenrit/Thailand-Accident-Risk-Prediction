...
let trafficIndexCache: { data: TrafficIndex[]; timestamp: number } | null =
  null;
let trafficEventsCache: { data: TrafficEvent[]; timestamp: number } | null =
  null;
...
export const getBangkokTrafficIndex = async (): Promise<number> => {
  ...
  const bangkok = trafficData.find(
    (item) =>
      item.province.toLowerCase().includes("bangkok") ||
...
/**
 * Fetch traffic events - DISABLED (returns empty array)
 * This was generating mock data. Real events come from Longdo API via event-manager.ts
 */
export const fetchTrafficEvents = async (bounds?: {
  north: number;
  south: number;
  east: number;
  west: number;
  ...
  // Real traffic events should come from authenticated Longdo Event API
  console.log("⚠️ traffic-service.ts events disabled (was mock data)");
  return [];
};
 ...
 */
const generateEventsFromTrafficData = (
  ...
  trafficData
    ...
    .forEach((item, index) => {
      ...
      const bangkokCoords = [
        ...
        { lat: 13.765, lng: 100.5377 }, // Ratchada
        { lat: 13.7368, lng: 100.5604 }, // Ekkamai
        { lat: 13.7245, lng: 100.493 }, // Sathorn
      ...
      if (item.index >= 8) {
        ...
        const eventTypes: TrafficEvent["type"][] = [
          "accident",
          "breakdown",
          "construction",
        ];
