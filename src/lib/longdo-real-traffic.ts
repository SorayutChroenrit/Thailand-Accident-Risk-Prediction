...
export async function scanAreaForRealTraffic(
  ...
  // Scan major points first (but limit to avoid rate limits)
  const pointsToCheck = majorPoints.slice(0, 3); // Only check first 3 points to avoid rate limits
  console.log(
    `📍 Checking ${pointsToCheck.length} major road points (reduced to avoid rate limits)...`,
  );
